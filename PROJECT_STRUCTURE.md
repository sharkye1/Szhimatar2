...
│   │   ├── RenderModeSelector.tsx         # Визуальный селектор CPU/GPU/Duo с градиентами
│   │   ├── RenderModeSelector.css         # Стили с анимациями для RenderModeSelector
│   │   ├── StatisticsPanel.tsx            # Панель статистики рендера
│   │   ├── PresetManager.tsx              # Управление пользовательскими пресетами
│   │   ├── FfmpegManager.tsx              # Управление FFmpeg путями
│   │   ├── FFmpegManagerTest.tsx          # Тестовый компонент для FFmpeg
│   │   ├── VideoInfoTest.tsx              # Тестовый компонент для видео-информации
│   │   └── VideoGuide.tsx                 # Гайд по использованию
│   │
...
│   │   ├── RenderService.ts               # Управление очередью рендера и прогрессом
│   │   ├── RenderScheduler.ts             # Планировщик для Duo Mode (CPU/GPU слоты)
│   │   ├── StatisticsService.ts           # Агрегация и хранение статистики
│   │   ├── FFmpegFinder.ts                # Поиск FFmpeg в системе
│   │   └── __tests__/                     # Unit-тесты для сервисов
│   │       └── RenderScheduler.test.ts    # 35 тестов для RenderScheduler
│   │
│   ├── 📁 hooks/                          # React хуки
│   │   ├── useRenderQueue.ts              # Хук для работы с очередью рендера
│   │   ├── useStatistics.ts               # Хук для статистики
│   │   └── useFFmpegRenderer.tsx          # Хук для FFmpeg рендеринга
│   │
│   ├── 📁 utils/                          # Утилиты и вспомогательные функции
│   │   ├── logger.ts                      # Система логирования (write_log в Rust)
│   │   ├── statistics.ts                  # Управление статистикой сжатий
│   │   └── ffmpeg.ts                      # FFmpeg утилиты и билдеры команд
│   │
...
│   ├── DUO_MODE_GUIDE.md                  # Руководство по Duo Mode
│   ├── STATISTICS_GUIDE.md                # Руководство по статистике
│   ├── STATISTICS_IMPLEMENTATION.md       # Детали реализации статистики
│   ├── HARDWARE_OVERRIDE_GUIDE.md         # Полное руководство по Hardware Override
│   ├── HARDWARE_OVERRIDE_QUICKSTART.md    # Быстрый старт Hardware Override
│   ├── HARDWARE_OVERRIDE_RISKS.md         # Риски и меры предосторожности
│   ├── HARDWARE_OVERRIDE_CHEATSHEET.md    # Шпаргалка по Hardware Override
│   ├── FFMPEG_*.md                        # Документация по FFmpeg интеграции
│   └── docs/RENDER_SYSTEM.md              # Архитектура системы рендеринга 
...
| **Components** | Переиспользуемые компоненты (RenderModeSelector, StatisticsPanel, PresetManager, MotionScreen, AppVersion) |
| **Services** | Бизнес-логика (RenderService, RenderScheduler, StatisticsService, FFmpegFinder) |
| **Hooks** | React хуки (useRenderQueue, useStatistics, useFFmpegRenderer) |
| **Utils** | Вспомогательные функции (логирование, статистика, FFmpeg утилиты) |
...
| `detect_hardware_info()` | Определение CPU/GPU вендоров (Intel/AMD/NVIDIA) |
| `check_gpu_compatibility()` | Проверка наличия NVENC через FFmpeg |
| `run_ffmpeg_render()` | Запуск FFmpeg процесса с progress tracking |
| `stop_ffmpeg_render()` | Остановка конкретного FFmpeg процесса |
| `stop_all_renders()` | Остановка всех активных рендеров |
| `get_video_duration()` | Получение длительности видео через FFprobe |
| `load_ffmpeg_config()` | Загрузка путей FFmpeg/FFprobe |
| `save_ffmpeg_config()` | Сохранение путей FFmpeg/FFprobe |
| `find_ffmpeg_in_path()` | Поиск FFmpeg в системном PATH |
| `find_ffmpeg_common_locations()` | Поиск FFmpeg в стандартных папках |
| `load_statistics()` | Загрузка статистики из `stats/stat.json` |
| `save_statistics()` | Сохранение статистики |
